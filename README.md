...
| `delegate-first.js` | workflow lib | `smart()` / `read()` helpers that pick the cheapest tier for a Workflow stage (worker → Sonnet, reader → Haiku) | Pasted into your own Workflow scripts | `manufacture.js` carries its own tier-keyed variant of the same idea (cheap → Haiku, code / adversarial → Sonnet, native → Sonnet in-session) |
