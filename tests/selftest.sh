...
WT="$T/wt"; mkdir -p "$WT/src/components" "$WT/design" && echo "gitdir: /elsewhere" > "$WT/.git" && echo mock > "$WT/design/page.html"
expect "design: git worktree (.git file) finds root design/" 0 design-source-gate.sh "$(write_in "$WT/src/components/Page.tsx" "$UI")"
...
# restore STOP, then break the pack one way at a time
mv "$P/components/C1.md.bak" "$P/components/C1.md"
valid() { bash "$H/manuf-pack-validate.sh" "$P" >/dev/null 2>&1; }
valid && ok "pack: restored pack VALID again" || bad "pack: restored pack VALID again"
cp "$P/manifest.json" "$T/man.bak"
echo '{"components":[{"id":"C1","spec":"components/C1.md","tier":"code","order":1},{"id":"C1","spec":"components/C1.md","tier":"code","order":2}],"ui_screens":[]}' > "$P/manifest.json"
valid; [ $? = 1 ] && ok "pack: duplicate component ids INVALID" || bad "pack: duplicate component ids INVALID"
echo 'just prose' > "$P/other.md"
echo '{"components":[{"id":"C1","spec":"other.md","tier":"code","order":1}],"ui_screens":[]}' > "$P/manifest.json"
valid; [ $? = 1 ] && ok "pack: manifest spec outside components/ checked for fields" || bad "pack: manifest spec outside components/ checked for fields"
mkdir -p "$P/design" && echo m > "$P/design/s1.html" && echo '# brief' > "$P/design/TASTE-BRIEF.md" && printf 'Locked: 2026-01-01 by tester\n' > "$P/ui-flow.md"
echo '{"components":[{"id":"C1","spec":"components/C1.md","tier":"code","order":1}],"ui_screens":[{"id":"S1","name":"x","mock":"design/s1.html","data_contract":"design/../spec.md"}]}' > "$P/manifest.json"
valid; [ $? = 1 ] && ok "pack: data_contract escaping design/ INVALID" || bad "pack: data_contract escaping design/ INVALID"
cp "$T/man.bak" "$P/manifest.json"
cp "$P/components/C1.md" "$T/c1.bak"
printf '## PLACEMENT\na:1\n## CODE\n```js\nconst rows = sampleData\n```\n## COMMANDS\nx\n## EXPECTED\nx\n## PRECONDITIONS\nx\n## POSTCONDITIONS\nx\n## STOP\nx\n' > "$P/components/C1.md"
valid; [ $? = 1 ] && ok "pack: dummy data inside CODE block INVALID" || bad "pack: dummy data inside CODE block INVALID"
printf '## PLACEMENT\na:1\n## CODE\nx\n## COMMANDS\n```sh\ngrep -rn sampleData src/\n```\n## EXPECTED\nx\n## PRECONDITIONS\nx\n## POSTCONDITIONS\nx\n## STOP\nx\n' > "$P/components/C1.md"
valid && ok "pack: dummy token in a COMMANDS grep is fine" || bad "pack: dummy token in a COMMANDS grep is fine"
cp "$T/c1.bak" "$P/components/C1.md"
TR="$T/pack.jsonl"; transcript "$TR" user "assemble it"; transcript "$TR" assistant "Blocked. Say manuf-pack-ok to bypass."
expect "pack hook: bypass word from assistant/hook text ignored" 2 manuf-pack-validate.sh "$(bash_in 'manufacture assemble' /tmp "$TR")"
transcript "$TR" user "manuf-pack-ok"
expect "pack hook: bypass word from the user honoured"          0 manuf-pack-validate.sh "$(bash_in 'manufacture assemble' /tmp "$TR")"
...
RO="$T/ro-pack"; mkdir -p "$RO/.qa" && chmod 500 "$RO/.qa"
bash "$S" write manuf-qa "$RO" A >/dev/null 2>&1; [ $? = 2 ] && ok "stamp: failed write is an error, not STAMPED" || bad "stamp: failed write is an error, not STAMPED"
chmod 700 "$RO/.qa"
